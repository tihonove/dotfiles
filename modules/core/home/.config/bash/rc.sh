...
# ~/.local/bin — туда modules/*/update ставят внешние тулзы (starship, fzf и пр.).
...
#
# Идёт ПЕРЕД ble.sh, и это не косметика. Конфиг ble.sh (~/.config/blesh/init.sh)
# выполняется в момент source и там же решает, подключать ли интеграцию с fzf —
# проверкой `type -P fzf`. Стоял этот блок ниже, и в PATH на тот момент не было
# ~/.local/bin: fzf, поставленный туда установщиком, не находился, интеграция
# молча пропускалась, и C-r/C-t/M-c не работали вовсе.
#
# Локально баг не проявлялся и потому жил долго: терминал наследует PATH от
# десктопной сессии, где ~/.profile уже отработал. Вылезает он там, где шелл
# стартует с чистого листа, — ssh и контейнеры, то есть ровно там, где чинить
# неудобнее всего.
case ":$PATH:" in
    *":$HOME/.local/bin:"*) ;;
...
# Ставится в ~/.local/share/blesh через modules/core/update, конфиг —
# ~/.config/blesh/init.sh.
#
# Грузить надо рано, а attach делать в самом конце (см. низ файла):
...
# Дроп-ины модулей: ~/.config/bash/rc.d/*.sh. Собственного входа в ~/.bashrc
# у модулей нет — маркированные блоки install.sh подключают только core (этот
# файл и prompt.sh). Всё, что модулю нужно в интерактивном шелле (обычно пара
# export'ов), он кладёт файлом в свой home/.config/bash/rc.d/, и оно приезжает
# симлинком как любой другой payload. Порядок — лексикографический, как в
# любом *.d; пока файлов единицы, префиксы-номера не нужны.
for _rc in "$HOME/.config/bash/rc.d"/*.sh; do
    [[ -f $_rc ]] && . "$_rc"
done
unset _rc
