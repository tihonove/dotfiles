...
# ── Миграции ─────────────────────────────────────────────────────────────────
#
# Изменение в репе приезжает на машину само, изменение в состоянии ВНЕ репы —
# нет. Профиль, бинарники в ~/.local/bin, блоки в ~/.bashrc: репа туда при
# `git pull` не дотягивается. Переименуй модуль — и на всех остальных машинах
# профиль ссылается на несуществующий модуль, а `./update.sh` там годен только
# на то, чтобы напечатать про это ошибку.
#
# Миграция — скрипт, который приводит машинное состояние в соответствие с уже
# приехавшей репой. Требования к нему ровно три:
#
#   1. срабатывает по СОСТОЯНИЮ, а не по отметке о применении. Поэтому никакого
#      файла «какие миграции уже прошли» нет вовсе: чинить нечего — миграция
#      молчит и выходит с нулём. Это же делает её безопасной на свежей машине,
#      где чинить нечего никогда;
#   2. идемпотентна: гоняется на каждой установке, а не один раз;
#   3. уважает --dry-run: печатает, что сделала бы, и не трогает ничего.
#
# Запускаются на путях, которые и так меняют машину: обычная установка, add и
# --dry-run (этому передаётся флаг, менять он ничего не станет). `list`,
# `--check` и `remove` состояние не чинят — читать его и молча править по
# дороге инструмент не должен.
#
# Запускаются ДО чтения профиля — профиль они и правят. Сбой миграции установку
# не роняет по той же причине, что и сбой установщика тулов: раскладку это не
# отменяет, а вот отказ целиком — это ровно та поломка обновления, от которой
# миграции и заводились. Громко печатается, уходит в код возврата.
MIGRATIONS_DIR="$DOTFILES_DIR/migrations"

declare -a MIGRATIONS_FAILED=()

run_migrations() {
    local mig name out rc

    [[ -d "$MIGRATIONS_DIR" ]] || return 0

    for mig in "$MIGRATIONS_DIR"/*; do
        [[ -f "$mig" && -x "$mig" ]] || continue
        name="${mig##*/}"

        # Вывод копится, а не льётся: у молчаливой миграции (обычный случай)
        # не должно быть даже заголовка.
        rc=0
        out="$(
            DOTFILES_DIR="$DOTFILES_DIR" \
            DOTFILES_PROFILE="$PROFILE" \
            DOTFILES_HOME="$TARGET_DIR" \
            DOTFILES_DRY_RUN="$DRY_RUN" \
            "$mig" 2>&1
        )" || rc=$?

        if ((rc)); then
            MIGRATIONS_FAILED+=("$name")
            {
                echo "⚠  миграция $name упала (код $rc), установка продолжается:"
                [[ -n "$out" ]] && sed 's/^/     /' <<<"$out"
            } >&2
            echo >&2
        elif [[ -n "$out" ]]; then
...
# ── Отсев модулей ────────────────────────────────────────────────────────────
#
# Модуль бывает неустановим по причинам, которые остальных модулей не касаются:
# его нет в репе (профиль отстал от переименования) или в системе нет команд из
# requires. Раньше и то и другое было отказом ВСЕЙ установки — и любая такая
# мелочь на другой машине превращала ./update.sh в кирпич: core с bash и tmux
# не приезжал из-за того, что в профиле осталось имя переехавшего модуля.
#
# Теперь неустановимый модуль просто выпадает из плана. Тихим пропуск при этом
# не становится — ради чего проверка и заводилась: он печатается здесь, ещё раз
# в конце установки и уводит код возврата в ненулевой. «Поставил конфиг sway на
# машину без sway» по-прежнему не происходит; происходит «поставил всё
# остальное и сказал, чего не смог».
#
# Отсев идёт до build_plan: план строится уже без выпавших модулей, и их update
# тоже не запускается.
...
screen_modules() {
    ...
    for m in "${MODULES[@]}"; do
        if [[ ! -d "$MODULES_DIR/$m" ]]; then
            SKIPPED+=("«$m»: такого модуля в репе нет — профиль отстал; ./install.sh remove $m")
            continue
        fi

        missing="$(module_missing_cmds "$m")"
        if [[ -n "$missing" ]]; then
            SKIPPED+=("«$m»: в системе нет — $missing; поставь их или ./install.sh remove $m")
            continue
        fi

        # Модуль без home/ подозрителен, только если он и скриптов не объявил.
        # Модуль, который есть ровно установщик и ничего больше (так жил diode,
        # пока не принёс кусок конфига git), — нормальная штука, и ругаться на
...
# меняем: задача — либо разрешить раскладку целиком, либо честно отказать.
#
# Здесь остались только претензии к ФАЙЛАМ: каталог на месте файла, два модуля
# на одну цель, ~/.bashrc-каталог. Они означают, что раскладка испортит данные,
# и пропустить их, в отличие от сломанного модуля, нельзя ни при каких
# обстоятельствах — это уже не «обновление не доехало», а «обновление сломало».

declare -a ERRORS=() WARNINGS=()
declare -A SEEN_PATH=()

preflight() {
    local i m rel target dir acc comp owner
    local -a parts
    local -A rel_owner=()
...
# Второй файл после ~/.bashrc, который НЕ симлинк, и по той же причине: в него
# пишут чужие. Управляемая часть лежит по XDG-пути ~/.config/git/config, а этот
# файл — локальный слой поверх неё.
#
# Держать его существующим обязательно, и вот почему: `git config --global`
# пишет в ~/.gitconfig, только ЕСЛИ ТОТ ЕСТЬ, иначе — в ~/.config/git/config,
# то есть прямо в репу. Пустого файла достаточно, чтобы этого не случилось.
#
# Что записи в него реальны, видно на devsy-контейнере: там ~/.gitconfig уже
# заведён самим devsy, он кладёт туда свой credential-хелпер.
GITCONFIG="$HOME/.gitconfig"

ensure_gitconfig() {
    if [[ -e "$GITCONFIG" || -L "$GITCONFIG" ]]; then
        echo "  ~/.gitconfig: на месте (локальный слой)"
        return
    fi

    if ((DRY_RUN)); then
        echo "  ~/.gitconfig: создал бы пустой (локальный слой поверх ~/.config/git/config)"
        return
    fi

    cat > "$GITCONFIG" <<'EOF'
# Локальный слой git. Пуст намеренно.
#
# Управляемая часть — в ~/.config/git/config (симлинк из ~/.dotfiles). git
# читает её раньше этого файла, поэтому здесь перекрывается что угодно:
# рабочая почта, чужие credential-хелперы, машинные настройки.
#
# Сюда же пишет `git config --global`. Собственно, ради этого файл и создан:
# без него та же команда писала бы в управляемый конфиг внутри гит-репы.
#
# Пример рабочей личности:
#   [user]
#       email = you@work.example
EOF
    echo "  ~/.gitconfig: создан (локальный слой)"
}
...
case "${1-}" in
    ...
    add)
        ...
        run_migrations        # правят профиль, поэтому строго до его чтения
        read_profile
        profile_add "$2"
...
# Отсев до заголовка: в нём должен стоять список модулей, которые реально
# поедут, а не тот, что написан в профиле.
screen_modules
...
if ((NO_TOOLS)); then
    ...
    run_module_tools || rc=1
fi

# Итог одним местом: раскладка сделана, но что-то по дороге не доехало. Всё
# перечисленное здесь — то, что можно доделать потом, ничего не откатывая.
echo
if [[ ${#MIGRATIONS_FAILED[@]} -gt 0 ]]; then
    rc=1
    echo "⚠  миграции не отработали: ${MIGRATIONS_FAILED[*]}" >&2
    printf '     ./migrations/%s\n' "${MIGRATIONS_FAILED[@]}" >&2
fi
if [[ ${#SKIPPED[@]} -gt 0 ]]; then
    rc=1
    ((DRY_RUN)) && echo "⚠  модули будут пропущены, остальное поедет:" >&2 \
                || echo "⚠  модули пропущены, остальное поставлено:" >&2
    printf '     %s\n' "${SKIPPED[@]}" >&2
fi

if ((DRY_RUN)) && ((rc)); then
    echo "⚠  План проверен, но поедет не всё — см. предупреждения выше"
elif ((DRY_RUN)); then
    echo "✅ План проверен, изменений не вносилось"
elif ((rc)); then
    echo "⚠  Готово не целиком — см. предупреждения выше"
else
    echo "✅ Готово"
fi
exit "$rc"
